...
import {
  ...
  getTotalRowData,
  parseTTNDate,
  parseTTNNumberFromAttachmentName,
} from './utils';
...
export const ttnVerticalMask = {
  ...
  extractData: (buffer: Buffer, attachmentName?: string) => {
    ...
    const parsed = parseTTNDate(sheet.A17?.v);

    return {
      type: DocumentType.TTN,
      date: parsed?.date ?? new Date(),
      number: parseTTNNumberFromAttachmentName(attachmentName),
      supplierId: SupplierId.ALMIK,
